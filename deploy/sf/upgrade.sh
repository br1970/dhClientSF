...
sed -i "/<ApplicationManifest.*ApplicationTypeVersion/s/ApplicationTypeVersion=\".*\"/ApplicationTypeVersion=\"$BUILD_BUILDNUMBER\"/" dhClientPkg/ApplicationManifest.xml
...
sed -i "/<ServiceManifestRef.*ServiceManifestVersion/s/ServiceManifestVersion=\".*\"/ServiceManifestVersion=\"$BUILD_BUILDNUMBER\"/" dhClientPkg/ApplicationManifest.xml
...
sed -i "/<ServiceManifest.*Version/s/Version=\".*\"/Version=\"$BUILD_BUILDNUMBER\"/" dhClientPkg/dhClientPkg/ServiceManifest.xml
[ $? -eq 0 ] || { echo "Failed to update CodePackage, Exiting..."; exit 1; }
echo "---Updating ServiceManifest Completed"
echo "---Updating CodePackage.."
sed -i "/<CodePackage.*Version/s/Version=\".*\"/Version=\"$BUILD_BUILDNUMBER\"/" dhClientPkg/dhClientPkg/ServiceManifest.xml
[ $? -eq 0 ] || { echo "Failed to update CodePackage, Exiting..."; exit 1; }
echo "---Updating CodePackage Completed"
echo "---Updating ImageName.."
sed -i "s/<ImageName>.*<\/ImageName>/<ImageName>balduino\/dhclientsf:$BUILD_BUILDNUMBER<\/ImageName>/" dhClientPkg/dhClientPkg/ServiceManifest.xml
...
echo "================================================="
azure servicefabric application upgrade start --application-name fabric:/dhMobile --target-application-type-version $BUILD_BUILDNUMBER --rolling-upgrade-mode Monitored
